    ...
    try:
        ...
        if cur.fetchone()[0] == 0:
            catalog = [
                ...
                ("cheese-200g", "Amul Cheese Slices", "Dairy", 150.00, "Amul", "200g", "pack", json.dumps(["dairy", "sandwich"])),

                # Staples/Pantry
                ("atta-5kg", "Aashirvaad Whole Wheat Atta", "Staples", 245.00, "Aashirvaad", "5kg", "bag", json.dumps(["flour", "roti"])),
                ("atta-1kg", "Aashirvaad Whole Wheat Atta", "Staples", 55.00, "Aashirvaad", "1kg", "bag", json.dumps(["flour", "roti"])),
                ("rice-basmati-1kg", "India Gate Basmati Rice", "Staples", 160.00, "India Gate", "1kg", "bag", json.dumps(["rice", "premium"])),
                ("rice-sela-5kg", "Kohinoor Sella Rice", "Staples", 499.00, "Kohinoor", "5kg", "bag", json.dumps(["rice"])),
                ...
                ("refined-oil-1l", "Fortune Refined Sunflower Oil", "Cooking Oil", 210.00, "Fortune", "1L", "bottle", json.dumps(["oil", "cooking"])),
                ("ghee-patanjali-500g", "Patanjali Desi Ghee", "Dairy", 480.00, "Patanjali", "500g", "jar", json.dumps(["ghee", "premium"])),

                # Snacks & Instant
                ("maggi-masala", "Maggi 2-Minute Noodles", "Instant Food", 14.00, "Nestle", "70g", "pack", json.dumps(["snack", "noodles"])),
                ("maggi-curry", "Maggi Curry Noodles", "Instant Food", 14.00, "Nestle", "70g", "pack", json.dumps(["snack", "noodles"])),
                ("biscuits-marie", "Britannia Marie Gold", "Snacks", 35.00, "Britannia", "250g", "pack", json.dumps(["tea-time"])),
                ("biscuit-parle-g-400g", "Parle-G Biscuits", "Snacks", 55.00, "Parle", "400g", "pack", json.dumps(["tea-time", "kids"])),
                ("chips-lays", "Lays Magic Masala", "Snacks", 20.00, "Lays", "50g", "pack", json.dumps(["snack", "spicy"])),
                ("kurkure-60g", "Kurkure Masala", "Snacks", 20.00, "PepsiCo", "60g", "pack", json.dumps(["snack"])),
                ("chips-haldiram-aloo", "Haldiram Aloo Bhujia", "Snacks", 120.00, "Haldiram", "400g", "pack", json.dumps(["snack", "namkeen"])),

                # Beverages & Tea/Coffee
                ("tea-250g", "Red Label Tea", "Beverages", 140.00, "Brooke Bond", "250g", "pack", json.dumps(["chai", "tea"])),
                ("coffee-nescafe-50g", "Nescafe Classic", "Beverages", 150.00, "Nescafe", "50g", "jar", json.dumps(["coffee"])),
                ("tropicana-1l", "Tropicana Mango Juice", "Beverages", 160.00, "Tropicana", "1L", "tetra", json.dumps(["juice"])),
                ("masala-chai-mix-100g", "MDH Tea Masala", "Spices", 45.00, "MDH", "100g", "pack", json.dumps(["chai", "spice"])),

                # Breakfast & Cereals
                ("oats-quaker-500g", "Quaker Oats", "Breakfast", 200.00, "Quaker", "500g", "pack", json.dumps(["oats", "breakfast"])),
                ("cornflakes-500g", "Kellogg's Corn Flakes", "Breakfast", 220.00, "Kellogg's", "500g", "box", json.dumps(["breakfast"])),
                ("poha-500g", "Avalakki / Poha", "Staples", 80.00, "", "500g", "pack", json.dumps(["breakfast", "rice"])),
                ("rava-1kg", "Suji / Rava", "Staples", 95.00, "", "1kg", "pack", json.dumps(["idli", "dosa"])),

                # Masalas & Spices
                ("mdh-garam-masala-100g", "MDH Garam Masala", "Spices", 120.00, "MDH", "100g", "pack", json.dumps(["masala", "spice"])),
                ("turmeric-100g", "MDH Turmeric Powder", "Spices", 50.00, "MDH", "100g", "pack", json.dumps(["spice"])),
                ("chilli-powder-100g", "Everest Chilli Powder", "Spices", 85.00, "Everest", "100g", "pack", json.dumps(["spice", "hot"])),

                # Fresh Produce (Market Price estimates)
                ...
                ("banana-1dozen", "Fresh Bananas (1 dozen)", "Fruits", 60.00, "", "1 dozen", "dozen", json.dumps(["fruit"])),

                # Protein & Eggs/Meat (demo; include to show variety)
                ("eggs-6", "Farm Fresh Eggs (6 pcs)", "Protein", 60.00, "", "6 pcs", "pack", json.dumps(["eggs", "protein"])),
                ("chicken-500g", "Chicken (Fresh, 500g)", "Meat", 220.00, "", "500g", "pack", json.dumps(["meat", "non-veg"])),

                # Ready / Frozen / Convenience
                ("frozen-paratha-400g", "ITC Frozen Parathas", "Frozen", 120.00, "ITC", "400g", "pack", json.dumps(["frozen", "breakfast"])),
                ("instant-idli-mix-200g", "MTR Idli Mix", "Instant Food", 90.00, "MTR", "200g", "pack", json.dumps(["instant", "south"])),
                
                # Sweets & Mithai
                ("haldiram-soanpapdi-200g", "Haldiram Soan Papdi", "Sweets", 180.00, "Haldiram", "200g", "box", json.dumps(["sweet"])),

                # Misc (to demonstrate diverse tags)
                ("toothpaste-150g", "Colgate Toothpaste", "Personal Care", 95.00, "Colgate", "150g", "tube", json.dumps(["personal"])),
                ("detergent-1kg", "Surf Excel Detergent Powder", "Household", 240.00, "Surf Excel", "1kg", "pack", json.dumps(["cleaning"]))
            ]
            cur.executemany("""
...
class FoodAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            You are 'Shyam', a helpful assistant for 'Reliance Fresh', an Indian grocery store.
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("🚀 STARTING RELIANCE FRESH SHOP (Indian Context + Auto-Tracking)")
